	...
	function commandLineCommandHelp(command) {
		if (command === "readcfg") {
			...
			return [" * writecfg - write boot configuration",
				...
				"",
                "    <boot_config_text> := <global_settings> [ <boot_sequence> ]",
				"",
                "    <global_settings> := <monitor_status> <console_ioset> ",
				"",
                "    <monitor_status> := \"MONITOR_DISABLED\" | \"MONITOR_ENABLED\" ",
				"",
                "    <console> := \"DBGU\" | \"FLEXCOM0_USART_IOSET1\" | ... | \"FLEXCOM12_USART_IOSET1\"",
				"",
                "    <boot_sequence> := <boot_intf> | <boot_intf> | <boot_intf> | <boot_intf> | <boot_intf> | <boot_intf> | <boot_intf>",
				"",
                "    <boot_intf> := <sdmmc> | <qspi> | <nfc> | <spi>",
                "",
                "    <sdmmc> := \"SDMMC0\" | \"SDMMC1\" [ '_' <card_detect_pin> ]",
                "",
                ...
                "    <nfc_entry> := \"NFC\"",
				"",
                "    <spi> := <spi0_entry> | <spi1_entry> | <spi2_entry> | <spi3_entry> | <spi4_entry> | <spi5_entry>",
                "",
